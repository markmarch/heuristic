...
class Population(tours : List[Tour], generationSize : Int, eliminationRate : Double, mutationRate : Double) {
  val populationSize = tours.length
  val eliminationSize = (populationSize * eliminationRate).toInt
  ...
  private def createNewPopulation(individuals : List[Tour]) = {
    ...
    @tailrec def eliminate(list : List[Tour], count : Int) : List[Tour] = {
      if (count == 0)
        ...
        eliminate(list.take(r-1) ::: list.drop(r), count - 1)
      }
    }

    val afterSelection = eliminate(individuals, eliminationSize)
    // println("natural selection finished." + afterSelection.length)
    ...
    println("optimal : " + result.head.fitness)
    result
  }
}
